defmodule Frac5.Pnm do
  def read_header(device, cline \\ "", pars \\ %{}) do
    extract = [:kind, :width, :height, :max] |>
      Enum.find(fn key -> not Map.has_key?(pars, key) end)
    if extract == nil do
      pars
    else
      line = if String.trim(cline) == "" do
        IO.read(device, :line)
      else
        cline
      end
      if extract == :kind do
        ...
        read_header(
	  device, String.slice(line, 2..-1//1), Map.put(pars, :kind, kind))
      else
  ...
  def read_gz!(path) do
    ...
    shape = case header.kind do
      "P5" ->
        {header.height, header.width}
      "P6" ->
        {header.height, header.width, 3}
    end
    data = Nx.from_binary(IO.binread(f, :eof), :u8) |>
      Nx.reshape(shape) |>
      Nx.reverse(axes: [0])
    File.close(f)
    data
  ...
  def render_pnm(grid, path) do
    {height, width, kind} = case Nx.shape(grid) do
      {h, w, 3} ->
        {h, w, :ppm}
      {h, w} ->
        {h, w, :pgm}
